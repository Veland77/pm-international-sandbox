...
-- One row means "the PO to this vendor for this order has actually been
-- issued" — the print document itself stays fully derived (PO number,
-- line items) and is never stored; this table exists only to turn
-- "generate the PO" into a real, one-time action instead of the document
-- simply existing on request. No "generated by" field: there's no
-- login/session concept in this app yet (HTTP Basic Auth is one shared
-- credential, not per-user identity), so only the date is tracked. The
-- unique constraint is what makes clicking "Generate" twice a no-op
-- rather than a duplicate record.
CREATE TABLE IF NOT EXISTS vendor_po_issuances (
  id INTEGER PRIMARY KEY,
  order_id INTEGER NOT NULL REFERENCES orders(id),
  supplier_id INTEGER NOT NULL REFERENCES suppliers(id),
