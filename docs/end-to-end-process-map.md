...
| 4 | **Compare Vendor Quotes & Select Winner per Line Item** | Purchasing | **Built** | `line_item_sourcing`. This is the step the original 17-step list didn't name explicitly, even though it's the core of what PM does — different vendors can win different line items on one deal. |
...
| 6 | **Select Freight Quote** | Purchasing | Partial | `freight_quotes` table exists; no comparison/selection UI yet. Second decision point the original list left implicit. |
...
| # | Step | Department | Status | Notes |
|---|------|-----------|--------|-------|
| 11 | Purchase Order Sent to Vendor | Purchasing | Partial | `orders` table links `po_id` → `supplier_quote_id`. No formal outbound PO document generated yet (should mirror the INQ print pattern). |
...
7. **Formal outbound PO document** — generated the same way the INQ document is, rather than an internal-only DB action.
